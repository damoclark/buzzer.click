...
Settings.prototype.validate = function() {
    if (this._hasTeams) {
        if (this._teamSize < 1) {
            return [false, 'Team size must be greater than 0 when using teams.'];
        }
        if (this._maxTeams < 1) {
            return [false, 'Max teams must be greater than 0 when using teams.'];
        }
        if (this._teamNames.length > 0) {
            ...
            if (this._teamNames.length !== uniqueNames.length) {
                return [false, 'Team names must be unique.'];
            }
        }
        if (this._maxTeams === constants.UNLIMITED && this._teamSize === constants.UNLIMITED) {
            return [false, 'Either number of teams or team size can be unlimited, not both'];
        }
    } else {
        if (this._teamSize > 0) {
            return [false, 'Team size must be 0 when not using teams.'];
        }
        if (this._maxTeams > 0) {
            return [false, 'Max teams must be 0 when not using teams.'];
        }
        if (this._teamNames.length > 0) {
            return [false, 'Team names cannot be used for a non-team based session.'];
        }
    }

    if (!this._hasTeams) {
        if (this._maxContestants < 1) {
           return [false, 'Max contestants must be greater than 0 when not using teams.'];
        }
    } else {
        if (this._maxContestants > 0) {
           return [false, 'Max contestants must be 0 when using teams.'];
        }
    }
