...
exports.listen = function(server) {
    ...
    io.on('connection', function(socket) {
        ...
        socket.on(messageConstants.CREATE_SESSION_MESSAGE, createSessionMessageHandler);
        socket.on(messageConstants.REJOIN_SESSION_MESSAGE, rejoinSessionMessageHandler);

    });
};

function updateObservers(session) {
    var message = messageFactory.create(messageConstants.OBSERVER_UPDATE_MESSAGE);
    message.populate(session);
    io.sockets.in(roomNameFactory.observers(session.id)).emit(messageConstants.OBSERVER_UPDATE_MESSAGE, message);
}
...
function createSessionMessageHandler(data, response) {
    ...
    var requestMessage = messageFactory.restore(data, messageConstants.CREATE_SESSION_MESSAGE);
    var session = sessions.add(host, requestMessage.settings);

    var responseMessage = messageFactory.create(messageConstants.CREATE_SESSION_RESPONSE_MESSAGE);
...
function rejoinSessionMessageHandler(data, response) {
    var requestMessage = messageFactory.restore(data, messageConstants.REJOIN_SESSION_MESSAGE);
    var session = sessions.getById(requestMessage.sessionId);
    var errorMessage = messageFactory.create(messageConstants.ERROR_MESSAGE);
    var successMessage = messageFactory.create(messageConstants.SUCCESS_MESSAGE);
