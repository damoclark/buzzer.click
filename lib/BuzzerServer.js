...
function contestantJoinRequestHandler(message, response) {
    ...
    if (session.settings.hasTeams) {
        contestantJoinTeamRequestHandler(requestMessage, responseMessage, session);
        return;
    }
    ...
    var addResponse = session.addContestant(contestant, inquireTeamLeaderCallbackHandler);
...
function inquireTeamLeaderCallbackHandler() {
    return false;
}

function contestantJoinTeamRequestHandler(requestMessage, responseMessage, session) {
