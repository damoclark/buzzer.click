...
exports.listen = function(server) {
    ...
    io.on('connection', function(socket) {
        ...
        socket.on(messageConstants.CREATE_SESSION_MESSAGE, function(message, response) {
            console.log('Host create game request. Message: `' + message + '`');

            var host = new Host();
            host.id = idUtility.generateParticipantId();

            var typeMessage = messageFactory.restore(messageConstants.CREATE_SESSION_MESSAGE, message);
            var session = sessions.add(host, typeMessage.Settings);
